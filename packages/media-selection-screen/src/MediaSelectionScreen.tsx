...
class MediaSelectionScreen extends React.PureComponent<MediaSelectionScreenProps, MediaSelectionScreenState> {
    ...
    componentWillUnmount() {
        ...
        if (initialLeftSideBarHiddenState !== isLeftSideBarHidden && !initialNodeCreationDialogOpenState && !this.isInDialog()) {
            toggleSidebar();
        }
    }
    ...
    isInDialog = () => {
        return this.state.initialNodeCreationDialogOpenState
            || !!this.containerRef.current?.closest('[role="dialog"]');
    }
